...
build_folder=build_linear

## build data in "atom_style bond" style
gfortran $initial_file_location/$build_folder/chain_bond.f -o $initial_file_location/$build_folder/chain_bond.out
...
1.02            no distance less than this from site i-1 to i+1 (reduced unit)" > $data_location/$dirname$N/$build_folder/chain.def

$data_location/$dirname$N/$build_folder/chain_bond.out < $data_location/$dirname$N/$build_folder/chain.def > $data_location/$dirname$N/$build_folder/chain.dat
cp  $data_location/$dirname$N/$build_folder/chain.dat $data_location/$dirname$N/data.chain
printf "\tDatafile location: $data_location/$dirname$N/data.chain\n"

done
printf "\tbuild linear chain done: $(($nNs*$dN)) to $(($nNe*$dN)) every $dN\n\n"
